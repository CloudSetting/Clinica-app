...
export default function PagoExitosoPage() {
  ...
  // 1. Leemos los parámetros directamente de la URL
  const paymentId = searchParams.get('payment_id');
  const statusPago = searchParams.get('status');

  // 2. Calculamos el estado directamente sin usar useState ni useEffect
  const esExitoso = statusPago === 'approved';

  // Si el pago no fue aprobado, mostramos una pantalla de error amigable
  if (!esExitoso) {
    return (
